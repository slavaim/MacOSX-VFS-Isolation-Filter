...
*
*/
#ifndef VIFCOMMONHASHTABLE_H
#define VIFCOMMONHASHTABLE_H
...
typedef enum _GHT_STATUS_CODE{
    GHT_OK = 0x0,
    GHT_ALREADY_IN_HASH = (int)(-2),
    GHT_ERROR = (int)(-1)
} GHT_STATUS_CODE;
...
typedef struct s_hash_entry
{
#if defined( DBG )
    //
    // a shadow copy of the key
    //
    ght_hash_key_t       key_shadow;
    
#endif//DBG
    //
    // put the most usable fields on a single cache line
    //
    ...
    //
    // size of the alocation = sizeof(ght_hash_entry_t) + key_size
    //
    size_t               size;
 ...
 *         allocation failed.
 */
typedef void *(*ght_fn_alloc_t)(vm_size_t size, int how);

/**
 * Definition of the deallocation function pointers. This is simply the
 * same definition as @c mac_kfree().
 *
 * @param ptr a pointer to the region to free.
 */
typedef void (*ght_fn_free_t)(void *ptr, vm_size_t size );
...
typedef struct
{
    ...
    bool non_block;                    /* TRUE if the allocations shoud not block */
 ...
 *
 * @returns 
*           0  if the element could be inserted,
 *          -2 if the elemnt is already in the hash
 *          -1 if there is an error ( mac_kalloc failed )
 */
GHT_STATUS_CODE
ght_insert(
