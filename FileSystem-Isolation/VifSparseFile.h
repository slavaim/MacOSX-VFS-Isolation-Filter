...
#ifndef _VIFSPARSEFILE_H
#define _VIFSPARSEFILE_H
...
//
class VifSparseFile: public OSObject{
    
    OSDeclareDefaultStructors( VifSparseFile )
    
    friend class VifSparseFilesHashTable;
...
public:
    ...
    //
    // used by the ISOALTION subsystem
    //
    ...
    typedef struct _IdentificatioInfo{
        ...
        //
        // a ISOALTION covering vnode associated with the
        // sparse file, if exists
        //
        vnode_t             cawlCoveringVnode;
        
        //
        // a ISOALTION covered vnode unique ID, if exists, this is not the same as VifIOVnodeID,
...
private:
    ...
    static const int sFreeChunksArrayCapacity = ( VifSparseFile::BlockSize - sizeof( VifSparseFile::FreeChunkHeader) + sizeof(off_t) )/ sizeof( off_t );
...
#define VIF_SPARSE_FILE_SIGNATURE 0xABDDEFEF
        SInt64              signature;
...
private:
    ...
    //
    // a mount structutre for the related voume under the ISOALTION control,
    // must not be referenced, used only as a key to retrieve the VifVfsMntHook object
    //
    mount_t         mnt;
...
#if defined(DBG)
#define VIF_BTNODE_SIGNATURE    (0xABCD9977)
        uint32_t        signature;
#endif // DBG
    ...
    typedef struct _BTreeNode {
        ...
        //
        BTreeKeyEntry   keyEntries[ (2 * VifSparseFile::BTreeOrder) - 0x1 ];
        ...
        //
        off_t           children[ 2 * VifSparseFile::BTreeOrder ];
    ...
    //
    // if the offset is set to VifSparseFile::InvalidOffset a new node is allocated with the reserved space,
...
public:
    ...
    static VifSparseFile*  withPath( __in const char* sparseFilePath,
    ...
    //
    // sets a new ISOALTION related vnode, returns the old one
    //
    vnode_t exchangeIsolationRelatedVnode( __in vnode_t );
    ...
    //
    static off_t alignToBlock( __in off_t fileOffset ) { return (fileOffset & ~((off_t)VifSparseFile::BlockSize - 1)); }
    static off_t roundToBlock( __in off_t fileOffset ) { return (( fileOffset + (off_t)VifSparseFile::BlockSize - 1 ) & ~((off_t)VifSparseFile::BlockSize - 1)); }
    
    //
    // returns a referenced vnode related to the sparse file, used by the ISOALTION,
...
private:
    ...
    //
    static VifSparseFilesHashTable* withSize( int size, bool non_block );
...
public:
    ...
    //
    bool   AddEntry( __in unsigned char* coveredVnodeID/*char[16]*/, __in VifSparseFile* entry );
    ...
    //
    VifSparseFile*   RemoveEntry( __in const unsigned char* coveredVnodeID/*char[16]*/ );
    
    //
    // remove entry by object, i.e. removes the object from the hash
    // a caller must not acquire the lock
    //
    void RemoveEntryByObject( __in VifSparseFile* sparseFile );
    
    //
    // returns an entry from the hash table, the returned entry is referenced
    // if the refrence's value is "true"
    //
    VifSparseFile*   RetrieveEntry( __in const unsigned char* coveredVnodeID/*char[16]*/, __in bool reference = true );
    ...
    static VifSparseFilesHashTable* sSparseFilesHashTable;
};

//--------------------------------------------------------------------

#endif // _VIFSPARSEFILE_H
