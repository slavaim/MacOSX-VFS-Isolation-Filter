...
SInt32  gIsolationNameGenerator = 0x0;
SInt64  gIsolationWriteTimeStampSeed = 0x0;

static bool gFlushDataForTest = false;

//--------------------------------------------------------------------

#define super OSObject

OSDefineMetaClassAndStructors( VifCoveringFsd, OSObject )


//--------------------------------------------------------------------

typedef struct _VifRangeDescriptor{
...
VifCoveringFsd*  VifCoveringFsd::GlobalCoveringFsd = NULL;

//--------------------------------------------------------------------

bool VifCoveringFsd::InitCoveringFsd()
{
    VifCoveringFsd::GlobalCoveringFsd = VifCoveringFsd::withMount( NULL );
    assert( VifCoveringFsd::GlobalCoveringFsd );
    if( !VifCoveringFsd::GlobalCoveringFsd ){
...
VifCoveringFsd* VifCoveringFsd::GetCoveringFsd( __in mount_t mnt )
{
    assert( VifCoveringFsd::GlobalCoveringFsd );
    ...
    //
    return VifCoveringFsd::GlobalCoveringFsd;
    
}

//--------------------------------------------------------------------

void VifCoveringFsd::PutCoveringFsd( __in VifCoveringFsd* coveringFsd )
{
    //
...
VifCoveringFsd*
VifCoveringFsd::withMount( __in_opt mount_t mnt )
{
    VifCoveringFsd*   newCoveringFsd;
    
    newCoveringFsd = new VifCoveringFsd();
...
errno_t
VifCoveringFsd::VifFsdHookCreateCoveringVnodeInternal(
    __inout VifVnodCreateParams*  ap
    )
/*
 ...
 */
{
    ...
    assert( (VOPFUNC*)VIF_VOP_UNKNOWN_OFFSET != FltGetVnodeOpVector( ap->coveredVnode ) );
	...
	vfsp.vnfs_str 		 = "VifCoverVnode"; // this is only for debug purposes
	...
	vfsp.vnfs_vops 		 = FltGetVnodeOpVector( ap->coveredVnode ); // TO DO - get our own opvector!
	vfsp.vnfs_rdev 		 = 0;
	...
	if( error != 0 ){
		...
		assert ( "VifFsdHookCreateCoverVnode failed" );
		goto __exit;
...
bool
VifCoveringFsd::VifIsCoveringVnode( __in vnode_t vnode )
{
    VifIOVnode::VnodeType   vnType = VifIOVnode::kVnodeType_Native;
    VifIOVnode*             dldVnode;
    ...
    //
    assert( !( (VifIOVnode::kVnodeType_CoveringFSD == vnType) && NULL != vnode_fsnode( vnode ) ) );
    
    return ( VifIOVnode::kVnodeType_CoveringFSD == vnType );
}

//--------------------------------------------------------------------

vnode_t
VifCoveringFsd::VifGetCoveredVnode( __in vnode_t coveringVnode )
/*
 ...
 */
{
    ...
    dldVnode = VifVnodeHashTable::sVnodesHashTable->RetrieveReferencedIOVnodByBSDVnode( coveringVnode );
    if( dldVnode ){
        
        if( VifIOVnode::kVnodeType_CoveringFSD == dldVnode->dldVnodeType ){
...
void
VifCoveringFsd::VifSetUbcFileSize(
    __in VifCoveredVnode*  vnodesDscr,
    __in off_t   size,
    __in bool    forceSetSize
    )
{
    bool  purgeIsolationData = false;
    ...
    assert( VifIOVnode::kVnodeType_CoveringFSD == vnodesDscr->dldCoveringVnode->dldVnodeType );
    ...
    { // start of the locked region
        ...
        if( ( vnodesDscr->dldCoveringVnode->fileDataSize < size ) || forceSetSize ){
            ...
            //
            // if the file is being truncated the ISOALTION related cached data
            // past the new size must be purgedas it will never be flushed
            // and so will keep the file marked as dirty ( having unflushed
            // ISOALTION data ) and prevent the FSD unmounting
            //
            purgeIsolationData = ( vnodesDscr->dldCoveringVnode->fileDataSize > size );
    ...
    if( purgeIsolationData ){
        
        VifSparseFile*  sparseFile = NULL;
...
int
VifCoveringFsd::VifReplaceVnodeByCovering(
 ...
 */
{
    ...
    /*
    VifIOVnode*  dldVnodeToSetSparseFile = NULL;
    ...
    do{
        ...
        {// start of the lock
            ...
            //
            dldVnode = VifVnodeHashTable::sVnodesHashTable->CreateAndAddIOVnodByBSDVnode( *vnodeToCover );
            ...
            if( dldVnode && 0x1 == dldVnode->flags.vopHooked ){
                ...
                if( createCoveringVnode ){
                    ...
                    VifVnodCreateParams   createParams;
                    ...
                    error = VifFsdHookCreateCoveringVnodeInternal( &createParams );
                    ...
                    if( KERN_SUCCESS == error ){
                        ...
                        //
                        // TO DO - solve the name problem, the created VifIOVnode will have empty name, the covered
                        // VifIOVnode must be used to retrieve the name for log and etc
                        //
                        dldCoveringVnode = VifVnodeHashTable::sVnodesHashTable->CreateAndAddIOVnodByBSDVnode(createParams.coveringVnode,
                                                                                                             VifIOVnode::kVnodeType_CoveringFSD );
                        ...
                        if( dldCoveringVnode ){
                            ...
                            //
                            dldCoveringVnode->flags.isolationOn = dldVnode->flags.isolationOn;
                            ...
                            *vnodeToCover = createParams.coveringVnode;
...
#if DBG
                            {
                                ...
                                {// start of the lock
                                    assert( FltVnodeHooksHashTable::sVnodeHooksHashTable->RetrieveEntry( FltGetVnodeOpVector( dldCoveringVnode->vnode ), false ) );
                                }// end of the lock
                            ...
                            // TO DO - as there is no corresponding VifIOVnode the vnode will be passed
...
errno_t
VifCoveringFsd::createIsolationSparseFile( __in VifCoveredVnode*  coveredVnode, __in vfs_context_t  vfsContext )
{
    VifSparseFile*  sparseFile;
    errno_t         error = KERN_SUCCESS;
    VifIOVnode*     dldCoveringVnode = coveredVnode->dldCoveringVnode;
    ...
    do{
        ...
        if( KERN_SUCCESS == error ){
            ...
            if( KERN_SUCCESS == error && 0x0 == va.va_data_size ){
                ...
                // is not unique
                //
                error = VifVnodeSetsize( coveredVnode->coveredVnode, 0x1, 0x0, vfsContext);
                assert( KERN_SUCCESS == error );
                if( KERN_SUCCESS != error ){
                    
                    DBG_PRINT_ERROR(("VifVnodeSetsize() failed with an error(%u)\n", error));
                }
    ...
    VifMD5Init( &mdContext );
    //VifMD5Update ( &mdContext, (unsigned char*)nameStrRef->getCStringNoCopy(), nameStrRef->getLength() );
    VifMD5Update( &mdContext, (unsigned char*)&va.va_fileid, sizeof(va.va_fileid) );
    VifMD5Update( &mdContext, (unsigned char*)&va.va_fsid, sizeof(va.va_fsid) );
    VifMD5Final( &mdContext );
    ...
    {
        sparseFile = VifSparseFilesHashTable::sSparseFilesHashTable->RetrieveEntry( mdContext.digest, true );
    }
    ...
    if( !sparseFile ){
        ...
        char    name[ sizeof(TEST_ISOLATION_FILE_NAME) ];
        ...
        // create a new
        //
        snprintf( name, sizeof( name ), "%s%08x_%08x_%08x_%08x", TEST_ISOLATION_DIR,
                 *(int*)&mdContext.digest[0],*(int *)&mdContext.digest[4],*(int *)&mdContext.digest[8],*(int *)&mdContext.digest[12]);
        
        //
        // create the sparse file
        //
        sparseFile = VifSparseFile::withPath( (const char*)name,
    ...
    if( sparseFile ){
        ...
        //
        // mark the mount as controlled by the ISOALTION, do it before setting the dldCoveringVnode->sparseFile value
        //
        ...
        VifVfsMntHook*  mnt = VifVfsMntHook::withVfsMnt( vnode_mount( dldCoveringVnode->coveringVnode ) );
        assert( mnt );
        if( mnt ){
        ...
        sparseFile->exchangeIsolationRelatedVnode( dldCoveringVnode->vnode );
        ...
        if( deleteSparseFile ){
            ...
            { // strat of the lock
                ...
                assert( NULL == VifSparseFilesHashTable::sSparseFilesHashTable->RetrieveEntry( mdContext.digest, false ) );
                
                added = VifSparseFilesHashTable::sSparseFilesHashTable->AddEntry( mdContext.digest, sparseFile );
                assert( added );
                if( !added ){
                    
                    DBG_PRINT_ERROR(( "VifSparseFilesHashTable::sSparseFilesHashTable->AddEntry() failed for %s\n", nameStrRef->getCStringNoCopy() ));
                }
    ...
    nameStrRef->release();
    VIF_DBG_MAKE_POINTER_INVALID( nameStrRef );
...
int
VifCoveringFsd::VifReclaimCoveringVnode(
    __in vnode_t  coveringVnode
    )
{
    //
    // free the covered vnode ( it seems the following  call to RemoveIOVnodeByBSDVnode
    // is also doing this if the VifIOVnode's reference count drops to zero
    //
    
    assert( preemption_enabled() );
    assert( this->VifIsCoveringVnode( coveringVnode ) );
    ...
    if( dldVnode ){
        ...
        assert( VifIOVnode::kVnodeType_CoveringFSD == dldVnode->dldVnodeType );
        ...
        if( dldVnode->coveredVnode ){
            ...
            VifIOVnode*  dldCoveredVnode;
            
            dldCoveredVnode = VifVnodeHashTable::sVnodesHashTable->RetrieveReferencedIOVnodByBSDVnode( coveredVnode );
            if( dldCoveredVnode ){
                
                assert( VifIOVnode::kVnodeType_Native == dldCoveredVnode->dldVnodeType );
    ...
    //
    VifVnodeHashTable::sVnodesHashTable->RemoveIOVnodeByBSDVnode( coveringVnode );
    ...
    /*
     ...
     #11 0x46eadd59 in VifFsdInactiveHook (ap=0x3218bae0) at /work/DeviceLockProject/DeviceLockIOKitDriver/VifVNodeHook.cpp:453
     ...
     #16 0x46f3bf30 in VifCoveringFsd::VifReclaimCoveringVnode (this=0x5f26790, coveringVnode=0x7f277c0) at /work/DeviceLockProject/DeviceLockIOKitDriver/VifCoveringVnode.cpp:522
     #17 0x46ea3e7d in VifFsdReclaimHook (ap=0x3218bc14) at /work/DeviceLockProject/DeviceLockIOKitDriver/VifVNodeHook.cpp:532
...
void
VifCoveringFsd::releaseIOArgs( __in VifCfsdIOArgs* args)
{
    if( args->memDscArray ){
...
kern_return_t
VifCoveringFsd::rwData(
    __in VifCoveredVnode*  coveredVnode,
    __in VifCfsdIOArgs* args // a caller releases all associated resources
    )
{
...
#if defined( DBG )
    assert( VIF_CFSD_WRVN_SIGNATURE == args->signature );
#endif//DBG
    ...
    if( NULL == coveredVnode->dldCoveringVnode->sparseFile ){
        ...
        //
        RC = this->createIsolationSparseFile( coveredVnode, ( args->contextRef ? args->contextRef : vfs_context_current() ) );
    ...
    SInt64      timeStamp = OSIncrementAtomic64( &gIsolationWriteTimeStampSeed );
    ...
    for( unsigned int i = 0; i < count; ++i ){
        ...
        while( 0x0 != residual ){
            ...
            if( !map ){
                ...
                if( map ){
                    ...
                    map->unmap();
                    map->release();
                    VIF_DBG_MAKE_POINTER_INVALID( map );
                }
#endif//DBG
            ...
            if( sparseFile ){
                ...
                //
                VifSparseFile::IoBlockDescriptor*  dscrs = NULL;
                unsigned int                       dscrsCount;                     
                
                dscrsCount = ( VifSparseFile::alignToBlock( fileOffset + mappedLength + (VifSparseFile::BlockSize - 0x1)) -
                               VifSparseFile::alignToBlock( fileOffset ) ) / VifSparseFile::BlockSize;
                
                dscrs = (VifSparseFile::IoBlockDescriptor*)IOMalloc( dscrsCount * sizeof(*dscrs) );
                assert( dscrs );
                if( !dscrs ){
                ...
                for( int i = 0x0; i < dscrsCount; ++i ){
                    ...
                    if( 0x0 == i ){
                        ...
                        dscrs[i].fileOffset = VifSparseFile::alignToBlock( fileOffset + i*VifSparseFile::BlockSize );
                    }
                    ...
                    if( i < (dscrsCount - 0x1) ){
                        ...
                        // not the last entry
                        //
                        off_t  nextBlock = VifSparseFile::alignToBlock(dscrs[i].fileOffset + VifSparseFile::BlockSize);
                        
                        dscrs[i].size = (unsigned int)(nextBlock - dscrs[i].fileOffset);
                        assert( !( 0x0 != i && VifSparseFile::BlockSize != dscrs[i].size ) );
                ...
                for( int i = 0x0; i < dscrsCount; ++i ){
                    ...
                    if( KERN_SUCCESS == dscrs[i].error ){
                        ...
                        //
                        dscrs[i].fileOffset = VifSparseFile::InvalidOffset;
                        dscrs[i].size = 0x0;
                    ...
                    if( 0x1 == dscrs[i].flags.write ){
                        ...
                        offsetForRead = VifSparseFile::alignToBlock( dscrs[i].fileOffset );
                        lengthForRead = VifSparseFile::BlockSize;
                        
                        bufferForRead = IOMalloc( lengthForRead );
                        assert( bufferForRead );
                        if( !bufferForRead ){
                            
                            DBG_PRINT_ERROR(("bufferForRead = IOMalloc( VifSparseFile::BlockSize ) failed\n"));
                    ...
                    if( KERN_SUCCESS == RC && bytesLeft != lengthForRead ){
                        ...
                        if( 0x1 == dscrs[i].flags.write ){
                            ...
                            //
                            VifSparseFile::IoBlockDescriptor  writeDscr;
                    ...
                    if( bufferForRead != dscrs[i].buffer ){
                        ...
                        assert( lengthForRead == VifSparseFile::BlockSize );
        ...
        //
        assert( coveredVnode->dldCoveringVnode && coveredVnode->dldCoveringVnode->isControlledByIsolationFilter() );
        if( writeIO && coveredVnode->dldCoveringVnode && coveredVnode->dldCoveringVnode->isControlledByIsolationFilter() ){
            ...
            if( gFlushDataForTest ){
                ...
                //
                coveringVnodeRef = VifSparseFile::getCoveringVnodeRefBySparseFileID( sparseFile->sparseFileID() );
                assert( coveringVnodeRef == coveredVnode->coveringVnode ); // we inside a write VNOP, so vnode must be found

                dldCoveringVnode = VifVnodeHashTable::sVnodesHashTable->RetrieveReferencedIOVnodByBSDVnode( coveringVnodeRef );
                assert( dldCoveringVnode );
                if( dldCoveringVnode ){
                    
                    VifSparseFile*  sparseFileRef;
                    ...
                    if( sparseFileRef ){
                        ...
                        dldCoveredVnode = VifVnodeHashTable::sVnodesHashTable->RetrieveReferencedIOVnodByBSDVnode( dldCoveringVnode->coveredVnode );
                ...
                //
                assert( VifSparseFile::InvalidOffset == sparseFile->getOldestWrittenBlockOffset() );
                
            } // end if( gGlobalSettings.flushISOALTION ) of the flush test
        ...
        //
        if( KERN_SUCCESS != RC &&
            coveredVnode->dldCoveringVnode && coveredVnode->dldCoveringVnode->isControlledByIsolationFilter() &&
            sparseFile ){
            
            assert( !"ISOALTION is being bypassed" );
            DBG_PRINT_ERROR(("ISOALTION is being bypassed\n"));
            ...
            //
            // we should not fail requests because of the ISOALTION errors
            //
            RC = KERN_SUCCESS;
...
vnode_t
VifCoveringFsd::getCoveringVnodeRefByCoveredFileName( __in char* coveredFileName )
{
    errno_t       error;
    ...
    // disk is under the ISOALTION controll
    //
...
void
VifCoveringFsd::processWrite(
    __in VifCoveredVnode*  coverdVnode,
    ...
    )
{
...
#if defined( DBG )
    args.signature = VIF_CFSD_WRVN_SIGNATURE;
#endif//DBG
    ...
    { // strat of the locked region
        ...
        if( offset + originalResid > coverdVnode->dldCoveringVnode->fileDataSize ){
            ...
            // for a user if he deletes a drive before the ISOALTION processed the data, but I
            // believe that this is not an issue with the removeabale drives for many
            // reasons one of which is a proliferation of FAT32
            //
            RC = VifVnodeSetsize( coverdVnode->coveredVnode, offset + originalResid, IO_NOZEROFILL | IO_NOAUTH, gVfsContextSuser );
            if( KERN_SUCCESS != RC ){
                ...
                //
                RC = VifVnodeSetsize( coverdVnode->coveredVnode, offset + originalResid, 0x0, ap->a_context );
                if( KERN_SUCCESS != RC ){
                    
                    DBG_PRINT_ERROR(("VifVnodeSetsize() failed\n"));
                    goto __exit;
                } // if( KERN_SUCCESS != RC )
            } // if( KERN_SUCCESS != RC )
        ...
        for( int i = 0x0; i < numberOfBuffers; ++i ){
            ...
            VifRangeDescriptor    rangeDsc = { 0x0 };
            OSData*               rangeDscAsData;
            ...
            //
            memDscr->release();
            VIF_DBG_MAKE_POINTER_INVALID( memDscr );
...
void
VifCoveringFsd::processPagingIO(
    __in VifCoveredVnode* coverdVnode,
...
/*
 ...
 #10 0x3227ba9a in ?? () VIFriver
 #11 0x473c35dd in ?? () VIFriver
 ...
 #15 0x472dbe35 in ?? () VIFriver
 #16 0x472de878 in ?? () VIFriver
 #17 0x473c2714 in ?? () VIFriver
...
void
VifCoveringFsd::UnLockExclusive()
{
    assert( preemption_enabled() );
    assert( this->lock );
    IORecursiveLockUnlock( this->lock );
...
errno_t
VifCoveredVnode::getVnodeInfo( __in vnode_t coveringVnode )
{
    ...
    VifCoveringFsd*  fsd = VifCoveringFsd::GetCoveringFsd( vnode_mount( coveringVnode ) );
    assert( fsd );
    
    fsd->LockShared();
    {// start of the lock
        
        this->dldCoveringVnode = VifVnodeHashTable::sVnodesHashTable->RetrieveReferencedIOVnodByBSDVnode( coveringVnode );
        if( this->dldCoveringVnode ){
            
            if( VifIOVnode::kVnodeType_CoveringFSD == this->dldCoveringVnode->dldVnodeType ){
                ...
                this->dldCoveredVnode = VifVnodeHashTable::sVnodesHashTable->RetrieveReferencedIOVnodByBSDVnode( this->dldCoveringVnode->coveredVnode );
                ...
                assert( VifIOVnode::kVnodeType_Native == this->dldCoveredVnode->dldVnodeType );
                ...
                //
                assert( this->dldCoveringVnode->flags.isolationOn == this->dldCoveringVnode->flags.isolationOn );
...
VifVnopArgs<vnop_strategy_args_class>::getInputArguments( __inout vnop_strategy_args_class* apWrapper )
{
    ...
    this->fsd = VifCoveringFsd::GetCoveringFsd( vnode_mount( vnode ) );
    assert( this->fsd );
    if( !this->fsd )
        goto __exit;
...
__exit:
    ...
    if( error )
        this->VifVnopArgs<vnop_strategy_args_class>::putInputArguments( apWrapper );
...
__exit:
    ...
    if( error )
        this->VifVnopArgs<vnop_bwrite_args_class>::putInputArguments( apWrapper );
...
VifVnopArgs<vnop_exchange_args_class>::getInputArguments( __inout vnop_exchange_args_class* apWrapper )
{
    ...
    this->fsd = VifCoveringFsd::GetCoveringFsd( vnode_mount( apWrapper->ap->a_fvp ) );
    assert( this->fsd );
    if( !this->fsd )
        goto __exit;
...
__exit:
    ...
    if( error )
        this->VifVnopArgs<vnop_exchange_args_class>::putInputArguments( apWrapper );
...
__exit:
    ...
    if( error )
        this->VifVnopArgs<vnop_copyfile_args_class>::putInputArguments( apWrapper );
...
VifVnopArgs<vnop_mkdir_args_class>::getInputArguments( __inout vnop_mkdir_args_class* apWrapper )
{
    ...
    this->fsd = VifCoveringFsd::GetCoveringFsd( vnode_mount( apWrapper->ap->a_dvp ) );
    assert( this->fsd );
    if( !this->fsd )
        goto __exit;
...
__exit:
    ...
    if( error )
        this->VifVnopArgs<vnop_mkdir_args_class>::putInputArguments( apWrapper );
...
__exit:
    ...
    if( error )
        this->VifVnopArgs<vnop_mknod_args_class>::putInputArguments( apWrapper );
...
__exit:
    ...
    if( error )
        this->VifVnopArgs<vnop_rename_args_class>::putInputArguments( apWrapper );
...
__exit:
    ...
    if( error )
        this->VifVnopArgs<vnop_symlink_args_class>::putInputArguments( apWrapper );
...
__exit:
    ...
    if( error )
        this->VifVnopArgs<vnop_whiteout_args_class>::putInputArguments( apWrapper );
